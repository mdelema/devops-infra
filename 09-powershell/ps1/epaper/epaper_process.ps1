...
# Descripcion: Procesa archivos para el epaper de: Paula, Paula Casa, Tu Casa Aqui, SabadoShow y Suplementos ANP. Los sube vía FTP y envía notificación por correo
# Autor: mdelema
# Fecha de ejecución: Automática diaria
# ==============================================
...
function Write-DualLog {
    ...
    # Solo escribir a archivos de log si no estamos en medio del envi�o de correo para evitar recursion
        ...
        try {
            if (Test-Path $backupPath -PathType Container) {
                ...
                if ($latestBackupDir) {
                    ...
                    Write-DualLog "[WARN] No se encontraron directorios validos no vaci�os en backup para $publication en $backupPath"
                }
            } else {
...
"@
        ...
        foreach ($key in $backupPaths.Keys) {
            ...
            $linkBase = "https://edicionpapel.dominio/"
    ...
    if ($nombre -match "^(.+?)(\d{1,2})?\s*(\b\w+\b)\s*(\d{4})_WEB$") {
        ...
        if ($meses.ContainsKey($mesNombre)) {
            ...
            if ($nombre -like "*PAULA CASA*") {
                ...
                if (-not $dia) {
                    Write-DualLog "[WARN] No se encontro un di�a valido en '$nombre' para SABADO SHOW. Saltando."
                    $script:processSuccessful = $false
            ...
            if ($copySuccess -and ($flippingOffline -or $indexHtml)) {
                ...
                # Marcar la publicación como procesada
                if ($currentProcessStatusKey) {
                    $script:processedPublications[$currentProcessStatusKey] = $true
                }

                # Ubicación del Application.css
...
"@
        ...
        Write-DualLog "[WARN] Formato de carpeta '$nombre' no reconocido (esperado 'NOMBRE [DIA] MES AÑO_WEB'). Saltando."
        $script:processSuccessful = $false
    }
    Write-DualLog "----------------------------------------------------"
}
    ...
    foreach ($carpeta in $carpetasMes) {
        ...
        if (Test-Path $carpetaLocalMes -PathType Container) {
            ...
            if ($filesInFolder.Count -gt 0) {
                ...
                Write-DualLog "[INFO] Carpeta vaci�a encontrada para $($revista.Nombre) en '$carpetaLocalMes'. Saltando subida FTP."
                continue
            }
        } else {
